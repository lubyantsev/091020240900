...
def delete_button(button_id):
    button = Button.query.get(button_id)
    if button:
        schedule_id = button.schedule_id  # Получаем ID расписания из кнопки
        db.session.delete(button)
        db.session.commit()
        return redirect(url_for('edit_schedule', schedule_id=schedule_id))
    return redirect(url_for('home'))  # Если кнопка не найдена, можно перенаправить на главную страницу
